...
# 
# 【概要】
#  このページでは、「エスペラント文の汉字(ポーランド語)置換」で使う
#  最終的な JSON ファイル(合并3個JSONファイル)を生成する機能を提供します。
# 
# 【主な流れ】
#  1) CSVファイルを取り込み(エスペラント語根→汉字/ポーランド語訳の対応)
...
#=====================================================================
st.set_page_config(
    page_title="Narzędzie do generowania pliku JSON do zastępowania tekstu (znaki 汉字) w Esperanto",
    layout="wide"
)
st.title("Generowanie pliku JSON do zastępowania tekstu w Esperanto (znaki 汉字).")
st.write("---")

#=====================================================================
# 2) 概要説明 (使い方)
#=====================================================================
with st.expander("Instrukcje użytkowania (kliknij, aby rozwinąć)", expanded=True):
    st.markdown("""
    #### Wprowadzenie
    Na tej stronie można wygenerować (czasem może osiągać nawet 50 MB) plik JSON,
    używany przy ostatecznej zamianie tekstu w Esperanto (strona główna – main),
    a następnie pobrać go.

    Oto instrukcja korzystania:
    1. Prześlij (lub użyj domyślnego) **plik CSV** (np. lista rdzeni Esperanto i tłumaczeń na język polski).
    2. W razie potrzeby prześlij (lub użyj domyślnego) **plik JSON** (reguły dzielenia rdzeni, ustawienia tekstu po zastąpieniu itp.).
    3. Kliknij „Utwórz plik JSON do zastępowania”, a następnie pobierz
       wygenerowany **plik JSON do zastępowania**.

    Poniżej dostępne są także przykładowe pliki; można się na nich wzorować,
    aby zapoznać się z formatowaniem i konfiguracją niestandardową.
    """)
...
# ダウンロードするボタンを設けています。
#=====================================================================
with st.expander("Lista plików przykładowych (do pobrania)"):
    st.write("#### Lista plików przykładowych")

    # サンプルCSV１
    st.markdown("""
    **Plik CSV nr 1 (Lista rdzeni esperanckich z polskimi tłumaczeniami + adnotacje ruby)**
    Ten plik CSV zawiera wiersz po wierszu rdzeń esperancki oraz jego polskie tłumaczenie (z ewentualnymi adnotacjami ruby).
    Dopasowując się do tego formatu i wczytując plik CSV,
    można wygenerować plik JSON do zastępowania tekstu.
    """)
    file_path0 = './Appの运行に使用する各类文件/Lista rdzeni esperanckich z polskimi tłumaczeniami i adnotacjami ruby.csv'
    ...
    # サンプルCSV２
    st.markdown("""
    **Plik CSV nr 2 (Lista rdzeni esperanckich i znaków汉字, propozycja Mingeo z portalu Zhihu)** 
    W tym pliku CSV każdemu rdzeniowi esperanckiemu przypisano jeden lub kilka znaków 汉字.
    """)
    ...
    # サンプルCSV３
    st.markdown("""
    **Plik CSV nr 3 (Lista rdzeni esperanckich i znaków汉字)**
    Tutaj również przyporządkowano rdzenie esperanckie do znaków 汉字 (format CSV).
    """)
    ...
    # サンプルJSON１
    st.markdown("""
    **Plik JSON nr 1 (Ustawienia użytkownika dotyczące sposobu dzielenia rdzeni słów esperanckich)**
    **Zastosowanie**: Określa, jak dzielić słowa esperanckie na rdzenie
    oraz kiedy wstawiać formy pochodne (np. formy czasownika).
    Zobacz komentarze w pliku z przykładem.
    ( Przykład: `["am", "dflt", ["verbo_s1"]]` )
    """)
    ...
    # サンプルJSON２
    st.markdown("""
    **Plik JSON nr 2 (Ustawienia użytkownika dotyczące tekstu po zastąpieniu)**
    **Zastosowanie**: Służy do przypisywania znaków 汉字 lub specjalnych formatów
    do konkretnych słów w uzupełnieniu powyższych ustawień dzielenia rdzeni.
    (Najczęściej wystarczy plik CSV oraz JSON z dzieleniem rdzeni, więc ten plik jest opcjonalny.)
    """)
    ...
    st.markdown("""
    **Plik Excel nr 1 (Lista rdzeni esperanckich z adnotacjami w 14 językach)**  
    Zawiera 14 języków (japoński, chiński, koreański, angielski, rosyjski, hiszpański, włoski,
    francuski, niemiecki, arabski, hindi, polski, wietnamski, indonezyjski)
    wraz z dopasowanymi do nich tłumaczeniami/komentarzami ruby dla rdzeni esperanckich.
    """)
    ...
    # サンプルExcel
    st.markdown("""
    **Plik Excel nr 2 (Rdzenie esperanckie – japońskie tłumaczenia i adnotacje ruby, z poziomem opanowania)**  
    **Zastosowanie**: Przydatne, gdy chcemy dostosować tłumaczenia ruby dla określonych rdzeni esperanckich.
    Zawiera także informację o „poziomie opanowania”, opartą na podstawowym słowniku esperanto-japońskim.
    """)
...
#=====================================================================
options = {
    # ▼ここでキー(ユーザーに見える部分)をポーランド語へ翻訳▼
    "HTML (ze skalowaniem rozmiaru znaków Ruby)": "HTML格式_Ruby文字_大小调整",
    "HTML (ze skalowaniem znaków Ruby i zastępowaniem znaków 汉字)": "HTML格式_Ruby文字_大小调整_汉字替换",
    "HTML (podstawowy)": "HTML格式",
    "HTML (z zastępowaniem znaków 汉字)": "HTML格式_汉字替换",
    "Format w nawiasach": "括弧(号)格式",
    "Format w nawiasach (z zastępowaniem znaków 汉字)": "括弧(号)格式_汉字替换",
    "Proste zastępowanie": "替换后文字列のみ(仅)保留(简单替换)"
}

display_options = list(options.keys())
selected_display = st.selectbox('Wybierz format wyjściowy:', display_options)
format_type = options[selected_display]

# Przykładowa pętla tworząca sformatowany tekst
...
st.write("---")
st.markdown("**Przykład sformatowanego tekstu (zastosowano nagłówek HTML z ruby):**")
...
#=====================================================================
st.header("Krok 1: Przygotowanie pliku CSV")
st.markdown(
    """
### Wybierz **plik CSV** zawierający rdzenie esperanckie i informacje (znaki汉字 lub tłumaczenie), które chcesz uwzględnić.

---
    """
)

csv_choice = st.radio("Co zrobić z plikiem CSV?", ("Prześlij własny plik", "Użyj pliku domyślnego"))
csv_path_default = "./Appの运行に使用する各类文件/Lista rdzeni esperanckich z polskimi tłumaczeniami i adnotacjami ruby.csv"

CSV_data_imported = None

if csv_choice == "Prześlij własny plik":
    st.write("Prześlij dowolny plik CSV (zalecane kodowanie UTF-8).")
    ...
    if uploaded_file is not None:
        ...
        st.success("Pomyślnie przesłano plik CSV.")
    else:
        st.warning("Nie przesłano żadnego pliku CSV.")
        st.stop()

elif csv_choice == "Użyj pliku domyślnego":
    try:
        ...
        csv_buffer = StringIO(converted_text)
        CSV_data_imported = pd.read_csv(csv_buffer, encoding="utf-8", usecols=[0, 1])
        st.info("Używany będzie domyślny plik CSV.")
    except FileNotFoundError:
        st.error("Nie znaleziono domyślnego pliku CSV. Przerywam działanie.")
        st.stop()

st.write("Wczytywanie pliku CSV zakończone. Możesz przejść dalej.")
st.write("---")

#=====================================================================
# ステップ2: JSONファイル(語根分解法など)を準備
#=====================================================================
st.header("Krok 2: Przygotowanie plików JSON (dzielenie rdzeni, itp.)")
st.markdown("""
Możesz przesłać (lub użyć domyślnych) pliki JSON,
zawierające informacje o **sposobie dzielenia rdzeni esperanckich** lub
**własne teksty po zastąpieniu**.
Możesz też pobrać przykładowy plik JSON i go edytować, a następnie wczytać tutaj.
""")

json_choice = st.radio("1. Jak postąpić z plikiem JSON definiującym sposób dzielenia rdzeni esperanckich?", ("Prześlij własny plik JSON", "Użyj pliku domyślnego"))
json_path_default = "./Appの运行に使用する各类文件/世界语单词词根分解方法の使用者自定义设置.json"
custom_stemming_setting_list = None

if json_choice == "Prześlij własny plik JSON":
    uploaded_json = st.file_uploader("Prześlij plik JSON", type=['json'])
    if uploaded_json is not None:
        custom_stemming_setting_list = json.load(uploaded_json)
        st.success("Pomyślnie przesłano plik JSON.")
    else:
        st.warning("Nie przesłano żadnego pliku JSON.")
        st.stop()
elif json_choice == "Użyj pliku domyślnego":
    try:
        with open(json_path_default, "r", encoding="utf-8") as g:
            custom_stemming_setting_list = json.load(g)
        st.info("Używany będzie domyślny plik JSON (dzielenie rdzeni).")
    except FileNotFoundError:
        st.error("Nie znaleziono domyślnego pliku JSON.")
        st.stop()

json_choice2 = st.radio("2. Jak postąpić z plikiem JSON definiującym tekst po zastąpieniu?", ("Prześlij własny plik JSON", "Użyj pliku domyślnego"))
json_path_default2 = "./Appの运行に使用する各类文件/替换后文字列(汉字)の使用者自定义设置(基本上完全不推荐).json"
user_replacement_item_setting_list = None

if json_choice2 == "Prześlij własny plik JSON":
    uploaded_json = st.file_uploader("Prześlij drugi plik JSON", type=['json'])
    if uploaded_json is not None:
        ...
        st.warning("Nie przesłano żadnego pliku JSON.")
        st.stop()
elif json_choice2 == "Użyj pliku domyślnego":
    try:
        with open(json_path_default2, "r", encoding="utf-8") as g:
            user_replacement_item_setting_list = json.load(g)
        st.info("Używany będzie domyślny plik JSON (tekst po zastąpieniu).")
    except FileNotFoundError:
        st.error("Nie znaleziono domyślnego pliku JSON (tekst po zastąpieniu).")
        st.stop()
...
# ステップ3: 高度な設定 (並列処理)
#=====================================================================
st.header("Krok 3: Zaawansowane ustawienia (przetwarzanie równoległe)")
with st.expander("Otwórz ustawienia dotyczące przetwarzania równoległego"):
    st.write("""
    Tutaj możesz ustawić liczbę procesów używanych
    do równoległego przetwarzania przy generowaniu pliku JSON.
    Jeśli liczba rdzeni lub tekstu jest duża,
    może to przyspieszyć działanie, wykorzystując wiele rdzeni CPU.
    """)

    use_parallel = st.checkbox("Użyj przetwarzania równoległego", value=False)
    num_processes = st.number_input("Liczba procesów równoległych", min_value=2, max_value=6, value=5, step=1)

st.write("### Ostateczne utworzenie pliku JSON do zastępowania (przycisk)")

#=====================================================================
# 置換用JSONファイルを作成するボタン
#=====================================================================
if st.button("Utwórz plik JSON do zastępowania"):
    with st.spinner("Generowanie pliku JSON do zastępowania... Proszę czekać."):
        #-------------------------------------------------------------
        ...
        #-------------------------------------------------------------
        # (3) CSV_data_imported を用いて エスペラント語根→(汉字 or polskie tłumaczenie) を反映
        #-------------------------------------------------------------
        ...
        if use_parallel:
            ...
            for i, j in enumerate(E_stem_with_Part_Of_Speech_list):
                ...
                if i % 1000 == 0:
                    ...
                    progress_text.write(f"{current_count}/{total_items} elementów w trakcie przetwarzania...")

            progress_bar.progress(100)
            progress_text.write("Najdłuższa część przetwarzania osiągnęła 100%. (Potrwa jeszcze około 3~4 sekundy).")
        ...
        st.success("Pomyślnie zakończono generowanie listy do zastępowania!")

        st.download_button(
            label="Pobierz ostateczną listę zastępowania (połączenie 3 plików JSON)",
            data=download_data,
            file_name="Finalna_lista_zastępowania_(scalone3_JSON).json",
            mime='application/json'
        )
